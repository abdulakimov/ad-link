...
export class MetaSyncService {
  ...
  async sync(
    ...
    try {
      ...
      await this.upsertInsights(tenantId, acc.id, insights);
  ...
  private async upsertHierarchy(tenantId: string, adAccountId: string, h: RawHierarchy) {
    for (const c of h.campaigns) {
      const campaign = await this.db.$base.campaign.upsert({
        ...
        create: { tenantId, adAccountId, externalId: c.externalId, name: c.name, status: c.status ?? null },
        update: { name: c.name, status: c.status ?? null },
      });
      for (const s of c.adSets) {
        const adSet = await this.db.$base.adSet.upsert({
          ...
          create: { tenantId, campaignId: campaign.id, externalId: s.externalId, name: s.name, status: s.status ?? null },
          update: { name: s.name, status: s.status ?? null },
        });
        for (const a of s.ads) {
          ...
          await this.db.$base.ad.upsert({
            ...
            update: { name: a.name, status: a.status ?? null, creativeId },
          });
        }
      }
    }
  ...
  private async upsertInsights(tenantId: string, adAccountId: string, insights: RawInsight[]) {
